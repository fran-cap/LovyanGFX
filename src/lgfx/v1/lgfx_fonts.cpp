...
namespace lgfx
{
 inline namespace v1
 {
  ...
  // ---------------------------------------------------------------------------
  // GLCDfont column decode as a table.
  //
  // A GLCDfont column is exactly one byte, so its decomposition into runs is a
  // function of 256 inputs -- i.e. a table.  Each entry packs the column's
  // FOREGROUND runs as up to four (start, end) nibble pairs, one pair per byte,
  // lowest byte first, terminated by a zero byte (`end` is never 0 for a real
  // run, so zero is an unambiguous sentinel).  Four is the exact maximum: the
  // most runs an 8-bit column can hold is 0b01010101.
  //
  // This does more than replace the transition-word scan.  The scan had to walk
  // background runs too, because it discovered the run boundaries one at a
  // time; the table lists only the runs that emit, so a column's background
  // costs nothing at all.
  //
  // Everything below is written in the C++11 constexpr subset (a single return
  // statement, recursion instead of loops).  It has to be: the desktop harness
  // builds this TU as C++17, but the Arduino-ESP32 core builds it as **C++11**,
  // where a constexpr function containing a loop is a hard error.  The table
  // must compile on both.

  // Index of the lowest set bit -- a constexpr count-trailing-zeros.
  static constexpr uint32_t glcd_ctz(uint32_t v, uint32_t k)
  {
    return ((v >> k) & 1u) ? k : glcd_ctz(v, k + 1u);
  }

  // The next run boundary after `j`, exactly as the scan computes it:
  // the run ends are the set bits of the transition word.
  static constexpr uint32_t glcd_next(uint32_t trans, uint32_t j)
  {
    return (trans >> j) ? (j + 1u + glcd_ctz(trans >> j, 0u)) : 8u;
  }

  // The scan's foreground runs, packed.  `flg` is the polarity the scan carries
  // (it starts at bit 0 of the column and toggles at every boundary), so this
  // walks the identical boundary sequence and keeps the identical subset of it.
  static constexpr uint32_t glcd_scan_pack(uint32_t trans, uint32_t j, uint32_t flg, uint32_t shift)
  {
    return (j >= 8u)
         ? 0u
         : (flg ? ((((j | (glcd_next(trans, j) << 4)) << shift))
                    | glcd_scan_pack(trans, glcd_next(trans, j), 0u, shift + 8u))
                : glcd_scan_pack(trans, glcd_next(trans, j), 1u, shift));
  }

  // One table entry, derived from the same transition word the scan built:
  // trans = (line ^ (line >> 1)) & trmask, with trmask for fontHeight == 8.
  static constexpr uint32_t glcd_scan_runs(uint32_t b)
  {
    return glcd_scan_pack((b ^ (b >> 1)) & ((((1u << 8) - 1u) >> 1)), 0u, b & 1u, 0u);
  }

  // The table is *generated by* the transcribed scan, so there are no magic
  // numbers to get wrong -- but generation alone would not catch a packing bug
  // (a fifth run, or an `end` too wide for its nibble).  So the entries are also
  // decoded back into a bit mask and checked against the byte they came from,
  // exhaustively over all 256 inputs at build time: the `split_lut` / `gl_magic`
  // discipline.  A single wrong bit in a single entry fails the build.
#define GLCD_RUN_1(n)   glcd_scan_runs(n)
#define GLCD_RUN_4(n)   GLCD_RUN_1(n),      GLCD_RUN_1((n) + 1),  GLCD_RUN_1((n) + 2),  GLCD_RUN_1((n) + 3)
#define GLCD_RUN_16(n)  GLCD_RUN_4(n),      GLCD_RUN_4((n) + 4),  GLCD_RUN_4((n) + 8),  GLCD_RUN_4((n) + 12)
#define GLCD_RUN_64(n)  GLCD_RUN_16(n),     GLCD_RUN_16((n) + 16), GLCD_RUN_16((n) + 32), GLCD_RUN_16((n) + 48)
#define GLCD_RUN_256    GLCD_RUN_64(0),     GLCD_RUN_64(64),       GLCD_RUN_64(128),      GLCD_RUN_64(192)

  static constexpr uint32_t glcd_fg_runs[256] = { GLCD_RUN_256 };
...
#undef GLCD_RUN_1
  ...
  // Decode a packed entry back into the set of pixels it lights.
  static constexpr uint32_t glcd_bits_from(uint32_t r)
  {
    return r ? ((((1u << ((r >> 4) & 0xFu)) - 1u) & ~((1u << (r & 0xFu)) - 1u))
                | glcd_bits_from(r >> 8))
             : 0u;
  }

  // Halving recursion rather than a 256-deep chain, to stay well inside any
  // -fconstexpr-depth.
  static constexpr bool glcd_runs_ok(uint32_t lo, uint32_t hi)
  {
    return (hi - lo == 1u)
         ? (glcd_bits_from(glcd_fg_runs[lo]) == lo)
         : (glcd_runs_ok(lo, (lo + hi) / 2u) && glcd_runs_ok((lo + hi) / 2u, hi));
  }
  static_assert(glcd_runs_ok(0u, 256u),
                "GLCDfont run table does not decode back to the column it came from");
  ...
  size_t GLCDfont::drawChar(LGFXBase* gfx, int32_t x, int32_t y, uint16_t c, const TextStyle* style, FontMetrics* metrics, int32_t& filled_x) const
  {
    ...
    // One load per column group yields the whole foreground run list, so the
    // scan is gone and so are the background iterations that carried it.
    // A column shorter than 8 pixels is the same column with its high bits
    // cleared -- masking gives exactly the truncated run list the scan built,
    // so no second code path is needed for a non-8-pixel GLCDfont.
    const uint32_t fhmask = (fontHeight < 8) ? ((1u << fontHeight) - 1) : 0xFFu;
    uint32_t x1 = 0;
    int_fast8_t i = 0;
    do
    {
      ...
      x1 = (i * sx) >> 16;
      uint32_t w = x1 - x0;
      uint32_t r = glcd_fg_runs[line & fhmask];
      while (r)
      {
        uint32_t j0 = r & 0x0Fu;
        uint32_t j1 = (r >> 4) & 0x0Fu;
        r >>= 8;
        uint32_t y0 = (j0 * sy) >> 16;
        uint32_t y1 = (j1 * sy) >> 16;
